...
App({
  ...
  onLaunch() {//小程序初始化的时候得到openid；每个微信用户访问小程序都对应唯一一个openid
    ...
    if (!store.getItem('uid', "userInfo")) {
      wx.login({
        //regeneratorRuntime is not defined ==>微信小程序不允许在自己的回调使用async，需要下载库
        async success(res) {
          //拿到code 0231TgZd1jiizw0qucYd1RT6Zd11TgZB
          //1. 向微信服务器发起请求，用code换openID
          //#region 
          // wx.request({
          ...
          let code = res.code
          let loading = true
          const result = await http.post(api.getOpenId, { code }, { loading })
          store.setItem('uid', result.uid, "userInfo")
        }
      })
    };
